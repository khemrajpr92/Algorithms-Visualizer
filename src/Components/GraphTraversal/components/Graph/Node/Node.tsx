...
export const Node = (props: NodeProps) => {
  ...
  return (
    <g key={`${node.id.toString()},${node.x},${node.y}`} className={styles.nodeGroup}>
      <circle
      ...
      <text key={`${node.id}-(${node.x},${node.y})`} className={styles.nodeText} x={node.x} y={node.y + 5}>
        {node.id}
      </text>
    </g>
  );
